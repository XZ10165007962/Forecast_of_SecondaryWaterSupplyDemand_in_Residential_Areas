...
def split_data(data_, split_col, split_flag, label_col, feature_col):
	if split_col == "time_index":
		start = split_flag[0]
		end = split_flag[1]
		train_data = data_[(data_[split_col] >= start-3*24) & (data_[split_col] <= end-3*24)]
		val_data = data_[(data_[split_col] >= start-2*24) & (data_[split_col] <= end-2*24)]
		test_data = data_[(data_[split_col] >= start) & (data_[split_col] <= end)]
		...
		train_data = data_[data_[split_col] < split_flag - 1]
...
def get_result():
	all_data = pd.read_csv(conf.tmp_data_paht + "test_data.csv")
